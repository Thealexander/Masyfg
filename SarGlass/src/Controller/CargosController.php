...
class CargosController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Cargos->save($cargo)) {
                $this->Flash->success(__('Se ha creado el cargo'));

                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(__('intenta de nuevo, el registro no se pudo almancenar.'));
            }
        }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            $cargo = $this->Cargos->patchEntity($cargo, $this->request->data);
            if ($this->Cargos->save($cargo)) {
                $this->Flash->success(__('Moficaciones guardadas exitosamente.'));

                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(__('Intenta de nuevo, un error se produjo.'));
            }
        }
        $this->set(compact('cargo'));
        $this->set('_serialize', ['cargo']);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Cargos->delete($cargo)) {
            $this->Flash->success(__('Registr eliminado'));
        } else {
            $this->Flash->error(__('Intenta de nuevo, se ha producido un error.'));
        }
